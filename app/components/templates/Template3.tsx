...
export default function Template3({ data }: Template3Props) {
  ...
  return (
    <div className="bg-white shadow-xl rounded-lg overflow-hidden print:shadow-none max-w-3xl mx-auto p-8">
          ...
          {projects.map((proj) => (
            <div key={proj.id} className="project-item mb-4 break-inside-avoid">
              <div className="flex justify-between items-baseline flex-wrap gap-2">
                ...
                {proj.link && (
                  <a href={proj.link} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600">
                    Link
                  </a>
                )}
              </div>
          ...
          <ul className="space-y-1">
            {languages.map((lang) => (
              <li key={lang.id} className="break-inside-avoid">
                ...
                {lang.proficiency && <span className="text-gray-600 text-sm ml-1">({lang.proficiency})</span>}
              </li>
            ))}
          </ul>
        </section>
          ...
          {publications.map((pub) => (
            <div key={pub.id} className="publication-item mb-3 break-inside-avoid">
              ...
              <div className="flex justify-between items-baseline flex-wrap gap-2">
                ...
                {pub.link && <a href={pub.link} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600">Link</a>}
              </div>
